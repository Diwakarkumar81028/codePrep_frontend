...
function Homepage() {
  ...
  useEffect(() => {
    const fetchProblems = async () => {
      try {
        ...
        const actualProblemsArray = Array.isArray(data) 
          ? data 
          : (data?.problems || data?.data || []);
        setProblems(actualProblemsArray);
      } catch (error) {
    ...
    const fetchSolvedProblems = async () => {
      try {
        ...
        const actualSolvedArray = Array.isArray(data) 
          ? data 
          : (data?.solvedProblems || data?.data || []);
        setSolvedProblems(actualSolvedArray);
      } catch (error) {
  ...
  const filteredProblems = Array.isArray(problems) 
    ? problems.filter(problem => {
        ...
        const isSolved = Array.isArray(solvedProblems) && solvedProblems.some(sp => sp?._id === problem?._id);
    ...
    <div className="min-h-screen bg-base-200">
      ...
      <nav className="navbar bg-base-100 shadow-lg px-4">
        <div className="flex-1">
          <NavLink to="/" className="btn btn-ghost text-xl">Codeprep</NavLink>
        </div>
        <div className="flex-none gap-4">
          <div className="dropdown dropdown-end">
            ...
            <ul tabIndex={0} className="mt-3 p-2 shadow menu menu-sm dropdown-content bg-base-100 rounded-box w-52 z-[100]">
              ...
              <div key={problem._id} className="card bg-base-100 shadow-md">
                <div className="card-body p-4">
                  ...
                  <div className="flex gap-2 mt-2">
                    ...
                    {problem.tags && (
                      <div className="badge badge-sm badge-info text-white">
                        {problem.tags}
                      </div>
                    )}
                  </div>
